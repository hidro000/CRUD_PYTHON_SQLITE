...
def inserir_usuario():
    ...
    if nome and email: 
        conn = conectar()
        c = conn.cursor()
        c.execute('INSERT INTO usuarios(nome, email, endereco, telefone) VALUES(?, ?, ?, ?)',(nome , email, endereco, telefone))
        conn.commit()
        conn.close()
...
columns  =  ('ID', 'NOME', 'E-MAIL', 'TELEFONE', 'ENDERECO')
